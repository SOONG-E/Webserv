...
void ServerManager::setBlock(Client& client) {
  ...
  const Location location_block = server_block.findLocation(request.getUri());

  response.setServerBlock(server_block);
  response.setLocation(location_block);
...
void ServerManager::receiveRequest(Client& client) {
  ...
  try {
    ...
    if (request.isCompleted() == true) {
      setBlock(client);
      validateRequest(request, client.getResponse().getLocation());
